...
public class ClassExtendsArrowEdgeRenderVisitor extends
		AbstractArrowEdgeRenderVisitor {
	...
	void drawButt(Edge e) {
		...
		Pt offl = getOffsetPoint2(e, e.angle2() - getArrowAngle() / 2d,
				getArrowLength());
		Pt offr = getOffsetPoint2(e, e.angle2() + getArrowAngle() / 2d,
				getArrowLength());
		g.setStrokeStyle(style.getGWTStrokeColor()).strokeShape(
				new ShapeBuilder().drawLineSegment(offl.x, offl.y,
						offr.x, offr.y).build());
		// g.drawLine((int) offl.x, (int) offl.y, (int) offr.x, (int) offr.y);
	}

	Pt getMidPoint(Edge e) {
		return getOffsetPoint2(e, e.angle2(), getArrowLength()
				* Math.cos(getArrowAngle() / 2d));
	}
